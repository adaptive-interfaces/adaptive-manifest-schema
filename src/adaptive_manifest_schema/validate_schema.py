"""validate_schema.py - Validates schema/manifest-1.toml internal consistency.
...
ALLOWED_FIELD_TYPES = {"string", "boolean", "list[string]", "integer"}


def validate_schema_internal(schema: ManifestSchemaData) -> list[str]:
    """Validate internal consistency of schema/manifest-1.toml."""
    errors: list[str] = []

    sections: dict[str, Any] = cast(dict[str, Any], schema.get("section", {}))
    fields: dict[str, Any] = cast(dict[str, Any], schema.get("field", {}))
    classes: dict[str, Any] = cast(dict[str, Any], schema.get("class", {}))

    # all sections referenced in class definitions must exist
    ...
    # all fields listed in section allowed_fields must have field definitions
    ...
    # all field types must be from the allowed set
