...
from adaptive_manifest_schema.cli import build_parser, main
...
def test_main_validate_explicit_path_returns_0() -> None:
    repo_root = Path(__file__).parent.parent
    result = main(["validate", "--path", str(repo_root / "MANIFEST.toml")])
    assert result == 0


def test_main_validate_missing_path_returns_1(tmp_path: Path) -> None:
    result = main(["validate", "--path", str(tmp_path / "MANIFEST.toml")])
    assert result == 1
